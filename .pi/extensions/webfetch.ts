...
export default function webfetchExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet:
      "Fetch a URL over HTTP/HTTPS and return markdown, text, or HTML.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      ...
      timeout: Type.Optional(
        Type.Number({ description: "Timeout in seconds (max 120)." }),
      ),
    }),
    async execute(_toolCallId, params, signal) {
      ...
      try {
        ...
        if (contentLength && Number(contentLength) > MAX_RESPONSE_SIZE) {
          throw new Error(
            `Response too large (${formatSize(Number(contentLength))}, max ${formatSize(MAX_RESPONSE_SIZE)})`,
          )
        }
        ...
        if (arrayBuffer.byteLength > MAX_RESPONSE_SIZE) {
          throw new Error(
            `Response too large (${formatSize(arrayBuffer.byteLength)}, max ${formatSize(MAX_RESPONSE_SIZE)})`,
          )
        }
...
function htmlToText(html: string): string {
  ...
  out = out.replace(
    /<\/(p|div|section|article|main|aside|header|footer|li|ul|ol|h[1-6]|tr|table|blockquote)>/gi,
    "\n",
  )
